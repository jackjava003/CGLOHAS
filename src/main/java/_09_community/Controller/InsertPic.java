...
@WebServlet("/_09_community/InsertPic.do")
public class InsertPic extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String fileName;
		if (picSize == 0) {
			errorMsg.put("errorPicSize", " 【Please confirm the photo】");
		} else {
		...
		if (!errorMsg.isEmpty()) {
			if (from.equalsIgnoreCase("ENV") || from.equalsIgnoreCase("FOOD")) {
				RequestDispatcher rd = request
						.getRequestDispatcher("/_09_community/InsertImg.jsp?holderID=" + userID + "&from=" + from);
				rd.forward(request, response);
				return;
			} else {
		...
		HolderImageBean hib = new HolderImageBean(userID,from,picBlob,title);
		int count = -1;
		count = hmd.insertImage(hib);
		...
		if (count == 1) {
			...
			request.setAttribute("Errors", "Failed to Update");
			RequestDispatcher rd = request.getRequestDispatcher("/_05_ShoppingCart/CheckError.jsp");
			rd.forward(request, response);
			return;
		}
